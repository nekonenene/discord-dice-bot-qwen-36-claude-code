...
- `dice` ディレクトリを分けて、ダイスロールのロジックを実装している
- テストコードを書いている

## 悪かった点

- コーディング時、途中で Update に失敗し続けて迷走していた
- メンションでないときにも反応してしまう仕組み
- メッセージ全体を見ているため、肝心のメンションしたときにエラーを返す。おそらくメンションはメッセージ的には `<@1234567890>` のように数字を含む文字列になるためだと思われる

### やり取りの様子

> +++++++++++++ Qwen No. 1 +++++++++++++

エラー: invalid dice format: "+++++++++++++ Qwen No. 1 +++++++++++++" (expected NDSides)

> @shamiko-bot 20D100

エラー: dice count must be an integer between 1 and 100
